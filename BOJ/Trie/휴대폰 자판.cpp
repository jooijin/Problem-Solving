...
각 테스트 케이스의 첫째 줄에 사전에 속한 단어의 개수 N이 주어지며(1 ≤ N ≤ 105), 
...
struct Trie{
	...
	int find(string &s, int idx, int cnt){
		...
		if(node[s[idx] - 'a']->finish) cnt++;
		else{
			int check = 0;
			for(int i=0; i<26; i++){
				if(node[s[idx] - 'a']->node[i] != NULL)
					check++;
				if(check >= 2){
					cnt++;
					break;
		...
		//cout<<s[idx]<<" "<<cnt<<endl;
		return node[s[idx] - 'a'] -> find(s, idx+1, cnt);
	}
...
int main(){
	...
	while(cin>>n){
		...
		int sum = 0;
		for(int i=0; i<n; i++){
			//cout<<trie->find(v[i], 0, 0)<<endl;
			sum += trie->find(v[i], 0, 0);
		}
