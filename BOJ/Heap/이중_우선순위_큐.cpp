	...
	while(T>0){
		...
		for(int i=0; i<k; i++){
			...
			if(c == 'I'){
				...
				if(n == 1){
                    if(!maxH.empty()){
					    isIn[maxH.top().second] = false;
					    maxH.pop();
                    }
                    while(!maxH.empty() && !isIn[maxH.top().second])
				        maxH.pop();
			        while(!minH.empty() && !isIn[minH.top().second])
				        minH.pop();
				}
				else if(n == -1){
					if(!minH.empty()){
					    isIn[minH.top().second] = false;
					    minH.pop();
                    }
                    while(!maxH.empty() && !isIn[maxH.top().second])
				        maxH.pop();
		...
		if(!minH.empty() && !maxH.empty()) cout<<maxH.top().first<<" "<<minH.top().first<<endl;
