...
void bfs(){
	...
	while(!q.empty()){
		...
		q.pop();
		for(int i=0; i<4; i++){
			int nx = n.first+dx[i]; int ny = n.second+dy[i];
			if(nx<0 || N<=nx || ny<0 || N<=ny) continue;
			if(map[nx][ny] == -1 || map[n.first][n.second]+arr[nx][ny] < map[nx][ny]){
				map[nx][ny] = map[n.first][n.second]+arr[nx][ny];
...
int main(){
    ...
    while(true){
		...
		bfs();
		cout<<"Problem "<<idx<<": "<<map[N-1][N-1]<<endl;
		idx++;
	}
