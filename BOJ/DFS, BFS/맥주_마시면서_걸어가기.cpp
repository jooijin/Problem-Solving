...
void bfs(vector<pair<int, int>> c, bool *v, pair<int, int> curr){
    ...
    while(!q.empty()){
        ...
        q.pop();
        if(abs(rx - curr.first) + abs(ry - curr.second) <= 1000){
            ck = true;
            return;
        }
        for(int i=0; i<c.size(); i++){
            if(!v[i] && abs(c[i].first - curr.first) + abs(c[i].second - curr.second) <= 1000){
                v[i] = true;
                q.push(make_pair(c[i].first, c[i].second));
            }
        }
    }
}
