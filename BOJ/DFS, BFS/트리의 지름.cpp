...
void dfs(int s, int rad){
	for(int i=0; i<edge[s].size(); i++){
		if(!v[edge[s][i].first]){
			v[edge[s][i].first] = true;
			dfs(edge[s][i].first, rad+edge[s][i].second);
			v[edge[s][i].first] = false;
			if(rad+edge[s][i].second > result){
				result = rad+edge[s][i].second;
				radEnd = edge[s][i].first;
			}
		}
    }
}
